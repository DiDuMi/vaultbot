...
必填：
- `BOT_TOKEN`
- `PROJECT_CODE`（兼容回退到 legacy `TENANT_CODE`）
- `PROJECT_NAME`（兼容回退到 legacy `TENANT_NAME`）
- `VAULT_CHAT_ID`
- `DATABASE_URL`
- `REDIS_URL`
...
- `WEBHOOK_PATH`
- `WEBHOOK_SECRET`
- `OPS_TOKEN`（为运维接口 `/ops/project-check` 启用请求头鉴权：`x-ops-token`；`/ops/tenant-check` 仍保留兼容）
- `EXPECTED_TENANT_CODE`（发布前预检期望项目编码）
...
部署脚本会在条件满足时自动执行项目预检（优先 `scripts/preflight-project.js`，兼容回退到 `scripts/preflight-tenant.js`），用于阻断 `TENANT_CODE` 配置漂移。
...
```bash
curl -H "x-ops-token: <OPS_TOKEN>" http://127.0.0.1:3002/ops/project-check
```
...
- 应用与 Worker 在启动时会执行 `TENANT_CODE` 一致性保护，避免误写入新租户导致运营数据“归零”。
