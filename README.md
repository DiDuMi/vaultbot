...
- `WEBHOOK_PATH`
- `WEBHOOK_SECRET`
- `OPS_TOKEN`（为运维接口 `/ops/tenant-check` 启用请求头鉴权：`x-ops-token`）
- `EXPECTED_TENANT_CODE`（发布前预检期望租户编码）
...
部署脚本会在条件满足时自动执行租户预检（`scripts/preflight-tenant.js`），用于阻断 `TENANT_CODE` 配置漂移。
